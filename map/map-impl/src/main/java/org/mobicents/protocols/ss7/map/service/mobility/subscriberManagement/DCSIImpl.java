	...
	protected void _decode(AsnInputStream asnIS, int length)
		...
		while (true) {
			...
			case _TAG_dpAnalysedInfoCriteriaList:
				if (ais.getTagClass() != Tag.CLASS_CONTEXT_SPECIFIC || ais.isTagPrimitive())
					throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + 
							".dpAnalysedInfoCriteriaList: Parameter bad tag class or primitive",	MAPParsingComponentExceptionReason.MistypedParameter);
				...
				while (true) {
					...
					if (tag2 != Tag.SEQUENCE || ais2.getTagClass() != Tag.CLASS_UNIVERSAL || ais2.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ": bad tag or tagClass or is primitive when decoding dpAnalysedInfoCriteriaList",
					...
					if (this.dpAnalysedInfoCriteriaList.size() < 1 || this.dpAnalysedInfoCriteriaList.size() > 10)
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".oBcsmCamelTDPDataList: dpAnalysedInfoCriteriaList elements count must be from 1 to 10, found: " + this.dpAnalysedInfoCriteriaList.size(),
								MAPParsingComponentExceptionReason.MistypedParameter);
				}
				break;
			case _TAG_camelCapabilityHandling:
				if (ais.getTagClass() != Tag.CLASS_CONTEXT_SPECIFIC || !ais.isTagPrimitive())
					throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
							+ ".camelCapabilityHandling: Parameter is not primitive or bad tag class", MAPParsingComponentExceptionReason.MistypedParameter);
			...
			case _TAG_notificationToCSE:
				if (ais.getTagClass() != Tag.CLASS_CONTEXT_SPECIFIC || !ais.isTagPrimitive())
					throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
							+ ".notificationToCSE: Parameter is not primitive or bad tag class", MAPParsingComponentExceptionReason.MistypedParameter);
			...
			case _TAG_csiActive:
				if (ais.getTagClass() != Tag.CLASS_CONTEXT_SPECIFIC || !ais.isTagPrimitive())
					throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
							+ ".csiActive: Parameter is not primitive or bad tag class", MAPParsingComponentExceptionReason.MistypedParameter);
				this.csiActive = true;
				ais.readNull();
				break;
			default:
	...
	public void encodeData(AsnOutputStream asnOs) throws MAPException {
		...
		try {
			if (this.dpAnalysedInfoCriteriaList != null && (this.dpAnalysedInfoCriteriaList.size() < 1 || this.dpAnalysedInfoCriteriaList.size() > 10))
				throw new MAPException("Error while encoding" + _PrimitiveName + ": dpAnalysedInfoCriteriaList size must be from 1 to 10, found: "
