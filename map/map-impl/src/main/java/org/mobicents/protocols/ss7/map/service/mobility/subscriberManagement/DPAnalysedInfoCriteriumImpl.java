...
public class DPAnalysedInfoCriteriumImpl extends SequenceBase implements DPAnalysedInfoCriterium{
	...
	public DPAnalysedInfoCriteriumImpl(ISDNAddressString dialledNumber, long serviceKey,
	...
	protected void _decode(AsnInputStream asnIS, int length)
		...
		while (true) {
			...
			case 0:
				...
				if (tag != Tag.STRING_OCTET || ais.getTagClass() != Tag.CLASS_UNIVERSAL || !ais.isTagPrimitive())
					throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
							+ ".dialledNumber: Parameter is bad tag, tag class or not primitive", MAPParsingComponentExceptionReason.MistypedParameter);
			...
			case 2:
				if (tag != Tag.STRING_OCTET || ais.getTagClass() != Tag.CLASS_UNIVERSAL || !ais.isTagPrimitive())
					throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
							+ ".gsmSCFAddress: Parameter is bad tag, tag class or not primitive", MAPParsingComponentExceptionReason.MistypedParameter);
			...
			case 3:
				if (tag != Tag.INTEGER || ais.getTagClass() != Tag.CLASS_UNIVERSAL || !ais.isTagPrimitive())
					throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
							+ ".defaultCallHandling: Parameter is bad tag, tag class or not primitive", MAPParsingComponentExceptionReason.MistypedParameter);
			...
			default:
				...
				case Tag.CLASS_UNIVERSAL: {
					...
					case Tag.SEQUENCE:
						...
						+ _PrimitiveName + ".extensionContainer: Parameter extensionContainer is primitive",
