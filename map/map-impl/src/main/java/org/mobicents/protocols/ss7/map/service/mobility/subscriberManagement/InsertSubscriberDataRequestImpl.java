	...
	private void _decode(AsnInputStream ansIS, int length) throws MAPParsingComponentException, IOException, AsnException {
		...
		while (true) {
			...
			case Tag.CLASS_CONTEXT_SPECIFIC:
				...
				case _TAG_category:
					if (!ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ".category: is not primitive",
								MAPParsingComponentExceptionReason.MistypedParameter);
					...
					while (true) {
						...
						if (tag2 != Tag.STRING_OCTET || ais2.getTagClass() != Tag.CLASS_UNIVERSAL || !ais2.isTagPrimitive())
							throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
									+ ": bad bearerServiceCode element tag or tagClass or is not primitive ", MAPParsingComponentExceptionReason.MistypedParameter);
				...
				case _TAG_teleserviceList:
					...
					while (true) {
						...
						if (tag3 != Tag.STRING_OCTET || ais3.getTagClass() != Tag.CLASS_UNIVERSAL || !ais3.isTagPrimitive())
							throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
									+ ": bad teleserviceCode tag or tagClass or is not primitive ", MAPParsingComponentExceptionReason.MistypedParameter);
				...
				case _TAG_provisionedSS:
					if (ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ".provisionedSS: Parameter is primitive",
								MAPParsingComponentExceptionReason.MistypedParameter);
					...
					if (this.provisionedSS.size() < 1 || this.provisionedSS.size() > 30) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ": Parameter provisionedSS size must be from 1 to 30, found: " + this.provisionedSS.size(),
								MAPParsingComponentExceptionReason.MistypedParameter);
					}
					break;
				case _TAG_odb_Data:
				...
				case _TAG_roamingRestrictionDueToUnsupportedFeature:
					if (!ais.isTagPrimitive()) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".roamingRestrictionDueToUnsupportedFeature: is not primitive", MAPParsingComponentExceptionReason.MistypedParameter);
					}
					ais.readNull();
				...
				case _TAG_regionalSubscriptionData:
					if (ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ".regionalSubscriptionData: Parameter is primitive",
								MAPParsingComponentExceptionReason.MistypedParameter);
					...
					while (true) {
						...
						if (tag4 != Tag.STRING_OCTET || ais4.getTagClass() != Tag.CLASS_UNIVERSAL || !ais4.isTagPrimitive())
							throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
									+ ": bad regionalSubscriptionData tag or tagClass or is not primitive ", MAPParsingComponentExceptionReason.MistypedParameter);
					...
					if (this.regionalSubscriptionData.size() < 1 || this.regionalSubscriptionData.size() > 10) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ": Parameter regionalSubscriptionData size must be from 1 to 10, found: " + this.regionalSubscriptionData.size(),
								MAPParsingComponentExceptionReason.MistypedParameter);
					}
					break;
				case _TAG_vbsSubscriptionData:
					if (ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ".vbsSubscriptionData: Parameter is primitive",
								MAPParsingComponentExceptionReason.MistypedParameter);
					...
					if (this.vbsSubscriptionData.size() < 1 || this.vbsSubscriptionData.size() > 50) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ": Parameter vbsSubscriptionData size must be from 1 to 50, found: " + this.vbsSubscriptionData.size(),
								MAPParsingComponentExceptionReason.MistypedParameter);
					}
					break;
				case _TAG_vgcsSubscriptionData:
					if (ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ".vgcsSubscriptionData: Parameter is primitive",
								MAPParsingComponentExceptionReason.MistypedParameter);
					...
					if (this.vgcsSubscriptionData.size() < 1 || this.vgcsSubscriptionData.size() > 50) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ": Parameter vgcsSubscriptionData size must be from 1 to 50, found: " + this.vgcsSubscriptionData.size(),
								MAPParsingComponentExceptionReason.MistypedParameter);
					}
					break;
				...
				case _TAG_gprsSubscriptionData:
					if (ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".gprsSubscriptionData: is primitive", MAPParsingComponentExceptionReason.MistypedParameter);
				...
				case _TAG_roamingRestrictedInSgsnDueToUnsupportedFeature:
					if (!ais.isTagPrimitive()) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".roamingRestrictedInSgsnDueToUnsupportedFeature: is not primitive", MAPParsingComponentExceptionReason.MistypedParameter);
					}
					ais.readNull();
				...
				case _TAG_lmu_Indicator:
					if (!ais.isTagPrimitive()) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".lmu_Indicator: is not primitive", MAPParsingComponentExceptionReason.MistypedParameter);
					}
					ais.readNull();
				...
				case _TAG_istAlertTimer:
					if (!ais.isTagPrimitive()) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".istAlertTimer: is not primitive", MAPParsingComponentExceptionReason.MistypedParameter);
					}
				...
				case _TAG_superChargerSupportedInHLR:
					if (!ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".lcsInformation: is not primitive", MAPParsingComponentExceptionReason.MistypedParameter);
				...
				case _TAG_cs_AllocationRetentionPriority:
					if (!ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".csAllocationRetentionPriority: is not primitive", MAPParsingComponentExceptionReason.MistypedParameter);
				...
				case _TAG_ics_Indicator:
					if (!ais.isTagPrimitive()) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ".icsIndicator: is not primitive",
								MAPParsingComponentExceptionReason.MistypedParameter);
					}
				...
				case _TAG_csg_SubscriptionDataList:
					if (ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ".csg_SubscriptionDataList: Parameter is primitive",
								MAPParsingComponentExceptionReason.MistypedParameter);
					...
					if (this.csgSubscriptionDataList.size() < 1 || this.csgSubscriptionDataList.size() > 50) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ": Parameter csgSubscriptionDataList size must be from 1 to 50, found: " + this.csgSubscriptionDataList.size(),
								MAPParsingComponentExceptionReason.MistypedParameter);
					}
					break;
				case _TAG_ue_ReachabilityRequestIndicator:
					if (!ais.isTagPrimitive()) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".ue_ReachabilityRequestIndicator: is not primitive", MAPParsingComponentExceptionReason.MistypedParameter);
					}
					ais.readNull();
				...
				case _TAG_mme_Name:
					if (!ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".mmeName: bad tag or tag class or not primitive", MAPParsingComponentExceptionReason.MistypedParameter);
				...
				case _TAG_subscribedPeriodicRAUTAUtimer:
					if (!ais.isTagPrimitive()) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ".subscribedPeriodicRAUTAUtimer: is not primitive",
								MAPParsingComponentExceptionReason.MistypedParameter);
					}
				...
				case _TAG_vplmnLIPAAllowed:
					if (!ais.isTagPrimitive()) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".vplmnLIPAAllowed: is not primitive", MAPParsingComponentExceptionReason.MistypedParameter);
					}
					ais.readNull();
				...
				case _TAG_mdtUserConsent:
					if (!ais.isTagPrimitive()) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ".mdtUserConsent: is not primitive",
								MAPParsingComponentExceptionReason.MistypedParameter);
					}
				...
				case _TAG_subscribedPeriodicLAUtimer:
					if (!ais.isTagPrimitive()) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ".subscribedPeriodicLAUtimer: is not primitive",
								MAPParsingComponentExceptionReason.MistypedParameter);
					}
		...
		if (mapProtocolVersion >= 2) {
			...
			if (this.regionalSubscriptionData != null) {
				try {
					...
					throw new MAPException("AsnException when encoding " + _PrimitiveName + ": regionalSubscriptionData" + e.getMessage(), e);
				}
			}
		}
		...
		if (mapProtocolVersion >= 3) {
			...
			if (this.lmuIndicator) {
				try {
					...
					throw new MAPException("IOException when encoding " + _PrimitiveName +  " parameter lmuIndicator: " + e.getMessage(), e);
				} catch (AsnException e) {
					throw new MAPException("AsnException when encoding " + _PrimitiveName + " parameter lmuIndicator: " + e.getMessage(), e);
				}
			}
			...
			if (this.superChargerSupportedInHLR != null) {
				((AgeIndicatorImpl) this.superChargerSupportedInHLR).encodeAll(asnOs, Tag.CLASS_CONTEXT_SPECIFIC, _TAG_superChargerSupportedInHLR);
			}
			...
			if (this.csAllocationRetentionPriority != null) {
				((CSAllocationRetentionPriorityImpl) this.csAllocationRetentionPriority).encodeAll(asnOs, Tag.CLASS_CONTEXT_SPECIFIC, _TAG_cs_AllocationRetentionPriority);
			}
			...
			if (this.ueReachabilityRequestIndicator) {
				try {
					...
					throw new MAPException("IOException when encoding " + _PrimitiveName +  " parameter ueReachabilityRequestIndicator: " + e.getMessage(), e);
				} catch (AsnException e) {
					throw new MAPException("AsnException when encoding " + _PrimitiveName + " parameter ueReachabilityRequestIndicator: " + e.getMessage(), e);
				}
			}
			...
			if (this.subscribedPeriodicRAUTAUtimer != null) {
				try {
					asnOs.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, _TAG_subscribedPeriodicRAUTAUtimer, this.subscribedPeriodicRAUTAUtimer);
				} catch (IOException e) {
					throw new MAPException("IOException while encoding " + _PrimitiveName + " parameter subscribedPeriodicRAUTAUtimer", e);
				} catch (AsnException e) {
					throw new MAPException("IOException while encoding " + _PrimitiveName + " parameter subscribedPeriodicRAUTAUtimer", e);
				}
			}
			...
			if (this.vplmnLIPAAllowed) {
				try {
					...
					throw new MAPException("IOException when encoding " + _PrimitiveName +  " parameter vplmnLIPAAllowed: " + e.getMessage(), e);
				} catch (AsnException e) {
					throw new MAPException("AsnException when encoding " + _PrimitiveName + " parameter vplmnLIPAAllowed: " + e.getMessage(), e);
				}
			}
			...
			if (this.subscribedPeriodicLAUtimer != null) {
				try {
					asnOs.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, _TAG_subscribedPeriodicLAUtimer, this.subscribedPeriodicLAUtimer);
				} catch (IOException e) {
					throw new MAPException("IOException while encoding " + _PrimitiveName + " parameter subscribedPeriodicLAUtimer", e);
				} catch (AsnException e) {
					throw new MAPException("IOException while encoding " + _PrimitiveName + " parameter subscribedPeriodicLAUtimer", e);
				}
			}
