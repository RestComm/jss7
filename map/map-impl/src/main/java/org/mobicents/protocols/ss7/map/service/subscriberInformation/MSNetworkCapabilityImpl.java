 ...
 *
 */
public class MSNetworkCapabilityImpl implements MSNetworkCapability, MAPAsnPrimitive {
	...
	private void _decode(AsnInputStream ansIS, int length) throws MAPParsingComponentException, IOException, AsnException {
		...
		if (this.data.length < 1 || this.data.length > 8)
			throw new MAPParsingComponentException("Error when decoding " + _PrimitiveName + ": data length must be from 1 to 8, found: " + this.data.length,
					MAPParsingComponentExceptionReason.MistypedParameter);
	}
	...
	public void encodeData(AsnOutputStream asnOs) throws MAPException {
		...
		if (this.data.length < 1 || this.data.length > 12)
			throw new MAPException("Error when encoding " + _PrimitiveName + ": data length must be from 1 to 8, found: " + this.data.length);
