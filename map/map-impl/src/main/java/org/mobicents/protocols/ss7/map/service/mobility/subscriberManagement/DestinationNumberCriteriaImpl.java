...
public class DestinationNumberCriteriaImpl  extends SequenceBase implements DestinationNumberCriteria {
			...
			case Tag.CLASS_CONTEXT_SPECIFIC: {
				...
				case _TAG_matchType:
					if (!ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ".matchType: Parameter is not primitive ",
				...
				case _TAG_destinationNumberList:
					if (ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ".destinationNumberList: Parameter is primitive ",
								MAPParsingComponentExceptionReason.MistypedParameter);
					...
					while (true) {
						...
						if (tag2 != Tag.STRING_OCTET || ais2.getTagClass() != Tag.CLASS_UNIVERSAL || !ais2.isTagPrimitive())
							throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ": bad tag or tagClass or is primitive when decoding destinationNumberList",
									MAPParsingComponentExceptionReason.MistypedParameter);		
					...
					if (this.destinationNumberList.size() < 1 && this.destinationNumberList.size() > 10) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ": Parameter destinationNumberList size must be from 1 to 10, found: " + this.destinationNumberList.size(),
								MAPParsingComponentExceptionReason.MistypedParameter);
					}
					break;
				case _TAG_destinationNumberLengthList:
					if (ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ".destinationNumberLengthList: Parameter is primitive ",
								MAPParsingComponentExceptionReason.MistypedParameter);
					...
					while (true) {
						...
						if (tag3 != Tag.INTEGER || ais3.getTagClass() != Tag.CLASS_UNIVERSAL || !ais3.isTagPrimitive())
							throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ": bad tag or tagClass or is primitive when decoding destinationNumberLengthList",
									MAPParsingComponentExceptionReason.MistypedParameter);	
					...
					if (this.destinationNumberLengthList.size() < 1 && this.destinationNumberLengthList.size() > 3) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ": Parameter destinationNumberLengthList size must be from 1 to 3, found: " + this.destinationNumberLengthList.size(),
								MAPParsingComponentExceptionReason.MistypedParameter);
					}
					break;
				default:
	...
	public void encodeData(AsnOutputStream asnOs) throws MAPException {
		...
		if (this.destinationNumberList != null && (this.destinationNumberList.size() < 1 || this.destinationNumberList.size() > 10)) {
			throw new MAPException("Error while encoding " + _PrimitiveName + ": Parameter destinationNumberList size must be from 1 to 10, found: "
		...
		if (this.destinationNumberLengthList != null && (this.destinationNumberLengthList.size() < 1 || this.destinationNumberLengthList.size() > 3)) {
			throw new MAPException("Error while encoding " + _PrimitiveName + ": Parameter destinationNumberLengthList size must be from 1 to 3, found: "
					+ this.destinationNumberList.size());
		}
