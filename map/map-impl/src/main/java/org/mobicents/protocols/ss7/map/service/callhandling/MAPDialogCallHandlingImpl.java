 ...
 * 
 */
public class MAPDialogCallHandlingImpl extends MAPDialogImpl implements
		MAPDialogCallHandling {
	...
	protected MAPDialogCallHandlingImpl(MAPApplicationContext appCntx,
			Dialog tcapDialog, MAPProviderImpl mapProviderImpl,
			MAPServiceBase mapService, AddressString origReference,
	...
	public Long addSendRoutingInformationRequest(long mapProtocolVersion,
		...
		return this.addSendRoutingInformationRequest(_Timer_Default,
				...
				extensionContainer, alertingPattern, ccbsCall,
				supportedCCBSPhase, additionalSignalInfo, istSupportIndicator,
				prePagingSupported, callDiversionTreatmentIndicator,
				longFTNSupported, suppressVtCSI, suppressIncomingCallBarring,
				gsmSCFInitiatedCall, basicServiceGroup2, networkSignalInfo2,
				supressMTSS, mtRoamingRetrySupported, callPriority);
	}
	...
	public Long addSendRoutingInformationRequest(int customInvokeTimeout,
		...
		if (true) { // validate parameters here...
			SendRoutingInformationRequestImpl req = new SendRoutingInformationRequestImpl(
					...
					camelInfo, suppressionOfAnnouncement, extensionContainer,
					alertingPattern, ccbsCall, supportedCCBSPhase,
					additionalSignalInfo, istSupportIndicator,
					prePagingSupported, callDiversionTreatmentIndicator,
					longFTNSupported, suppressVtCSI,
					suppressIncomingCallBarring, gsmSCFInitiatedCall,
					basicServiceGroup2, networkSignalInfo2, supressMTSS,
	...
	public void addSendRoutingInformationResponse(long invokeId,
			...
			NumberPortabilityStatus nrPortabilityStatus, Integer istAlertTimer,
			SupportedCamelPhases supportedCamelPhases,
			OfferedCamel4CSIs offeredCamel4CSIs, RoutingInfo routingInfo2,
			ArrayList<SSCode> ssList2, ExtBasicServiceCode basicService2,
		...
		ReturnResultLast resultLast = this.mapProviderImpl.getTCAPProvider()
		...
		if (true) { // validate parameters here...
			SendRoutingInformationResponseImpl res = new SendRoutingInformationResponseImpl(
					...
					msisdn, nrPortabilityStatus, istAlertTimer,
					supportedCamelPhases, offeredCamel4CSIs, routingInfo2,
					ssList2, basicService2, allowedServices,
					unavailabilityCause, releaseResourcesSupported,
					gsmBearerCapability);
			AsnOutputStream aos = new AsnOutputStream();
			res.encodeData(aos);
