				...
				case _TAG_smsCamelTdpDataList:
					if (ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".smsCamelTdpDataList: Parameter is primitive", MAPParsingComponentExceptionReason.MistypedParameter);
					...
					if (this.smsCamelTdpDataList.size() < 1 || this.smsCamelTdpDataList.size() > 1) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ": Parameter smsCamelTdpDataList size must be from 1 to 1, found: " + this.smsCamelTdpDataList.size(),
								MAPParsingComponentExceptionReason.MistypedParameter);
					}
					break;
				case _TAG_camelCapabilityHandling:
					if (!ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ".camelCapabilityHandling: Parameter not primitive",
								MAPParsingComponentExceptionReason.MistypedParameter);
