...
public class OCSIImpl extends SequenceBase implements OCSI {
	...
	public OCSIImpl(ArrayList<OBcsmCamelTDPData> oBcsmCamelTDPDataList, MAPExtensionContainer extensionContainer, Integer camelCapabilityHandling,
	...
	protected void _decode(AsnInputStream asnIS, int length) throws MAPParsingComponentException, IOException, AsnException {
			...
			case 0:
				...
				while (true) {
					...
					if (tag2 != Tag.SEQUENCE || ais2.getTagClass() != Tag.CLASS_UNIVERSAL || ais2.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ": bad tag or tagClass or is primitive when decoding oBcsmCamelTDPDataList",
								MAPParsingComponentExceptionReason.MistypedParameter);			
					...
					if (this.oBcsmCamelTDPDataList.size() < 1 || this.oBcsmCamelTDPDataList.size() > 10)
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".oBcsmCamelTDPDataList: tBcsmCamelTDPData elements count must be from 1 to 10, found: " + this.oBcsmCamelTDPDataList.size(),
								MAPParsingComponentExceptionReason.MistypedParameter);
				}
				break;
		...
		if (num < 1) {
			throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
					+ ": Parament oBcsmCamelTDPDataList is mandatory but does not found", MAPParsingComponentExceptionReason.MistypedParameter);
		}
	}
	...
	public void encodeData(AsnOutputStream asnOs) throws MAPException {
		try {
			...
			if (this.oBcsmCamelTDPDataList.size() < 1 || this.oBcsmCamelTDPDataList.size() > 10)
				throw new MAPException("Error while encoding" + _PrimitiveName + ": oBcsmCamelTDPDataList size must be from 1 to 10, found: "
						+ this.oBcsmCamelTDPDataList.size());
