...
public class OBcsmCamelTdpCriteriaImpl  extends SequenceBase implements OBcsmCamelTdpCriteria{
	...
	protected void _decode(AsnInputStream asnIS, int length)
		...
		while (true) {
			...
			case Tag.CLASS_UNIVERSAL:
				...
				case Tag.ENUMERATED:
					if (!ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ".oBcsmTriggerDetectionPoint: not primitive",
			...
			case Tag.CLASS_CONTEXT_SPECIFIC:
				...
				case _TAG_destinationNumberCriteria:
					if (ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".destinationNumberCriteria: is primitive", MAPParsingComponentExceptionReason.MistypedParameter);
				...
				case _TAG_basicServiceCriteria:
					...
					if (ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".basicServiceCriteria: is primitive", MAPParsingComponentExceptionReason.MistypedParameter);
					...
					while (true) {
						...
						if (!(tag2 == ExtBasicServiceCodeImpl._ID_ext_BearerService || tag2 == ExtBasicServiceCodeImpl._ID_ext_Teleservice)
							...
							throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
									+ ".: extBasicServiceCode bad tag or tagClass or is not primitive ", MAPParsingComponentExceptionReason.MistypedParameter);
				...
				case _TAG_callTypeCriteria:
					if (!ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".callTypeCriteria: is not primitive", MAPParsingComponentExceptionReason.MistypedParameter);
					int code = (int) ais.readInteger();
				...
				case _TAG_oCauseValueCriteria:
					...
					if (ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".oCauseValueCriteria: is primitive", MAPParsingComponentExceptionReason.MistypedParameter);
					...
					while (true) {
						...
						if (tag2 != Tag.STRING_OCTET || ais3.getTagClass() != Tag.CLASS_UNIVERSAL || !ais3.isTagPrimitive())
							throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
									+ ": causeValue tag or tagClass or is not primitive ", MAPParsingComponentExceptionReason.MistypedParameter);
					...
					if (this.oCauseValueCriteria.size() < 1 || this.oCauseValueCriteria.size() > 5) {
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ": Parameter oCauseValueCriteria size must be from 1 to 5, found: "
		...
		if (this.oBcsmTriggerDetectionPoint == null)
			throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ": Parament oBcsmTriggerDetectionPoint is mandatory but does not found",
					MAPParsingComponentExceptionReason.MistypedParameter);
	...
	public void encodeData(AsnOutputStream asnOs) throws MAPException {
		...
		if (this.oCauseValueCriteria != null && (this.oCauseValueCriteria.size() < 1 || this.oCauseValueCriteria.size() > 5)) {
			throw new MAPException("Error while encoding " + _PrimitiveName + ": Parameter oCauseValueCriteria size must be from 1 to 5, found: "
					+ this.oCauseValueCriteria.size());
		}
		...
		try {
			...
			if (this.destinationNumberCriteria != null)
				((DestinationNumberCriteriaImpl) this.destinationNumberCriteria).encodeAll(asnOs, Tag.CLASS_CONTEXT_SPECIFIC, _TAG_destinationNumberCriteria);
