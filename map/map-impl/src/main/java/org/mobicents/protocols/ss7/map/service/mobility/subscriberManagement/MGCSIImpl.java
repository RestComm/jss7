...
public class MGCSIImpl extends SequenceBase implements MGCSI{
	...
	public MGCSIImpl(ArrayList<MMCode> mobilityTriggers,
	...
	protected void _decode(AsnInputStream asnIS, int length)
		...
		while (true) {
				...
				case Tag.SEQUENCE:
					if (ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
								+ ".mobilityTriggers: Parameter is primitive", MAPParsingComponentExceptionReason.MistypedParameter);
					...
					while (true) {
						...
						if (tag2 != Tag.STRING_OCTET || ais2.getTagClass() != Tag.CLASS_UNIVERSAL || !ais2.isTagPrimitive())
							throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
									+ ".mobilityTriggers.MMCode: bad tag or tagClass or is not primitive ", MAPParsingComponentExceptionReason.MistypedParameter);
			...
			case Tag.CLASS_CONTEXT_SPECIFIC: {
				...
				case _TAG_extensionContainer:
					...
					+ _PrimitiveName + ".extensionContainer: Parameter is primitive",
								MAPParsingComponentExceptionReason.MistypedParameter);
				...
				case _TAG_notificationToCSE:
					if (!ais.isTagPrimitive())
						throw new MAPParsingComponentException("Error while decoding " 
					+ _PrimitiveName + ".notificationToCSE: Parameter not primitive",
								MAPParsingComponentExceptionReason.MistypedParameter);
					ais.readNull();
		...
		if (this.mobilityTriggers == null) {
			throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ": Parament mobilityTriggers is mandatory but does not found",
					MAPParsingComponentExceptionReason.MistypedParameter);
		}
