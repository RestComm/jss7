...
public class SSCSIImpl extends SequenceBase implements SSCSI {
	...
	public SSCSIImpl(SSCamelData ssCamelData,
	...
	protected void _decode(AsnInputStream asnIS, int length)
			...
			case 0:
				if (ais.isTagPrimitive() || ais.getTagClass() != Tag.CLASS_UNIVERSAL || tag !=  Tag.SEQUENCE )
					throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
							+ ".ssCamelData: Parameter is primitive or bad tag or bad tag class", MAPParsingComponentExceptionReason.MistypedParameter);
			...
			default:
				...
				case Tag.CLASS_CONTEXT_SPECIFIC: {
					...
					case _TAG_notificationToCSE:
						if (!ais.isTagPrimitive())
							throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ".notificationToCSE: Parameter not primitive",
		...
		if (ssCamelData == null) {
			throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ": Parament ssCamelData is mandatory but does not found",
					MAPParsingComponentExceptionReason.MistypedParameter);
		}
	}
	...
	public void encodeData(AsnOutputStream asnOs) throws MAPException {
		if (this.ssCamelData == null) {
			throw new MAPException("Error while encoding " + _PrimitiveName + " the mandatory parameter ssCamelData is not defined");
		}
