...
public class SendIdentificationResponseImpl extends MobilityMessageImpl implements SendIdentificationResponse{
	...
	public SendIdentificationResponseImpl(IMSI imsi,
		...
		if (this.mapProtocolVersion >= 3) {
			...
			while (true) {
				...
				case Tag.CLASS_UNIVERSAL:
					...
					case Tag.SEQUENCE:
						if (ais.isTagPrimitive()) {
							throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ".authenticationSetList: is primitive",
									MAPParsingComponentExceptionReason.MistypedParameter);
						}
						this.authenticationSetList = new AuthenticationSetListImpl();
						((AuthenticationSetListImpl) this.authenticationSetList).decodeAll(ais);
			...
			if (this.imsi == null) {
				throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName + ": imsi is mandatory for MAP V2 but not found ",
						MAPParsingComponentExceptionReason.MistypedParameter);
			}
		}
	}
