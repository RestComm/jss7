...
public class CAMELSCIGPRSBillingChargingCharacteristicsImpl extends SequenceBase implements CAMELSCIGPRSBillingChargingCharacteristics {
	...
	protected void _decode(AsnInputStream asnIS, int length)
			...
			if (ais.getTagClass() == Tag.CLASS_CONTEXT_SPECIFIC) {
				...
				case _ID_aocGPRS:
					if (ais.isTagPrimitive())
						throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ".aocGPRS: Parameter is primitive",
				...
				case _ID_pdpID:
					if (!ais.isTagPrimitive())
						throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ".pdpID: Parameter is not primitive",
								CAPParsingComponentExceptionReason.MistypedParameter);
		...
		if (this.aocGPRS == null)
			throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ": aocGPRS is mandatory but not found",
					CAPParsingComponentExceptionReason.MistypedParameter);
	}
