...
public class EventReportGPRSRequestImpl  extends GprsMessageImpl  implements EventReportGPRSRequest{
	...
	public EventReportGPRSRequestImpl(GPRSEventType gprsEventType,
		...
		while (true) {
			...
			if (ais.getTagClass() == Tag.CLASS_CONTEXT_SPECIFIC) {
				...
				case _ID_miscGPRSInfo:
					if (ais.isTagPrimitive())
						throw new CAPParsingComponentException(
								"Error while decoding " + _PrimitiveName + ".miscGPRSInfo: Parameter is primitive",
								CAPParsingComponentExceptionReason.MistypedParameter);
				...
				case _ID_gprsEventSpecificInformation:
					if (ais.isTagPrimitive())
						throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ".gprsEventSpecificInformation: Parameter is primitive",
								CAPParsingComponentExceptionReason.MistypedParameter);
		...
		try {
			...
			if (this.gprsEventSpecificInformation != null){
				try {
					...
					throw new CAPException("AsnException while encoding " + _PrimitiveName + " parameter gprsEventSpecificInformation");
				}
			}
