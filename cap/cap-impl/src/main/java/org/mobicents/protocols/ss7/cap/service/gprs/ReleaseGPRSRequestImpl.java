...
public class ReleaseGPRSRequestImpl   extends GprsMessageImpl  implements ReleaseGPRSRequest {
	...
	private void _decode(AsnInputStream ansIS, int length) throws CAPParsingComponentException, IOException, AsnException, MAPParsingComponentException {
				...
				case _ID_gprsCause:
					if (!ais.isTagPrimitive())
						throw new CAPParsingComponentException(
								"Error while decoding " + _PrimitiveName + ".gprsCause: Parameter is not primitive",
								CAPParsingComponentExceptionReason.MistypedParameter);
		...
		if (this.gprsCause == null)
			throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ": parameter gprsCause is mandatory but not found",
					CAPParsingComponentExceptionReason.MistypedParameter);
	}
