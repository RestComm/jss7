...
public class ROTimeGPRSIfTariffSwitchImpl  extends SequenceBase implements ROTimeGPRSIfTariffSwitch{
	...
	public ROTimeGPRSIfTariffSwitchImpl(Integer roTimeGPRSSinceLastTariffSwitch,
			...
			if (ais.getTagClass() == Tag.CLASS_CONTEXT_SPECIFIC) {
				...
				case _ID_roTimeGPRSSinceLastTariffSwitch:
					if (!ais.isTagPrimitive())
						throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ".roTimeGPRSSinceLastTariffSwitch: Parameter is not primitive",
								CAPParsingComponentExceptionReason.MistypedParameter);
					this.roTimeGPRSSinceLastTariffSwitch = (int) ais.readInteger();
					break;
				case _ID_roTimeGPRSTariffSwitchInterval:
					if (!ais.isTagPrimitive())
						throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ".roTimeGPRSTariffSwitchInterval: Parameter is not primitive",
								CAPParsingComponentExceptionReason.MistypedParameter);
	...
	public void encodeData(AsnOutputStream asnOs) throws CAPException {
		...
		try {
			if (this.roTimeGPRSSinceLastTariffSwitch != null)
				asnOs.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, _ID_roTimeGPRSSinceLastTariffSwitch, this.roTimeGPRSSinceLastTariffSwitch.intValue());
			
			if (this.roTimeGPRSTariffSwitchInterval != null)
				asnOs.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, _ID_roTimeGPRSTariffSwitchInterval, this.roTimeGPRSTariffSwitchInterval.intValue());
