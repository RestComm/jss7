...
public class ExtensionFieldImpl implements ExtensionField, CAPAsnPrimitive {
	...
	private void _decode(AsnInputStream ansIS, int length) throws CAPParsingComponentException, IOException, AsnException {
		...
		while (true) {
			...
			case 0:
				...
				if (ais.getTagClass() != Tag.CLASS_UNIVERSAL || !ais.isTagPrimitive())
					throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ": Parameter 0 bad tag class or not primitive",
							CAPParsingComponentExceptionReason.MistypedParameter);
				switch (tag) {
				case Tag.INTEGER:
				...
				case Tag.OBJECT_IDENTIFIER:
					...
					break;
				default:
					throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ": Parameter 0 bad tag",
							CAPParsingComponentExceptionReason.MistypedParameter);
				}
				break;
			...
			default: {
				...
				case Tag.CLASS_UNIVERSAL: {
					if (tag == Tag.ENUMERATED) {
						...
						if (this.criticalityType == null) {
							throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ": Bad criticalityType value",
									CAPParsingComponentExceptionReason.MistypedParameter);
						}
					} else {
						throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ": Bad tag of the CLASS_UNIVERSAL field",
								CAPParsingComponentExceptionReason.MistypedParameter);
					}
				}
					break;
				...
				case Tag.CLASS_CONTEXT_SPECIFIC: {
					if (tag == _ID_value) {
						...
						if (ais.available() < len) {
							throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ": not enouph data for a value field",
									CAPParsingComponentExceptionReason.MistypedParameter);
						}
						this.data = new byte[len];
						ais.read(this.data);
					} else {
						throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ": Bad tag of the CLASS_CONTEXT_SPECIFIC field",
								CAPParsingComponentExceptionReason.MistypedParameter);
					}
				}
					break;
				...
				default:
					throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ": Parameter 1 or 2 has bad tag class",
							CAPParsingComponentExceptionReason.MistypedParameter);
				}
			}
			}
		...
		if (this.data == null)
			throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ": value field is mandatory but not found",
					CAPParsingComponentExceptionReason.MistypedParameter);
	}
	...
	public void encodeData(AsnOutputStream aos) throws CAPException {
		...
		try {
			if ((this.localCode == null && this.globalCode == null) || (this.localCode != null && this.globalCode != null))
				throw new CAPException("Error while decoding " + _PrimitiveName + ": at least localCode or globalCode field must not be null");
			if (this.data == null)
