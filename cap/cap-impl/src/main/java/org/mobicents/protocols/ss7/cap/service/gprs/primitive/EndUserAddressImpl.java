...
public class EndUserAddressImpl extends SequenceBase implements EndUserAddress{
	...
	public EndUserAddressImpl(PDPTypeOrganization pdpTypeOrganization,
	...
	protected void _decode(AsnInputStream asnIS, int length)
			...
			if (ais.getTagClass() == Tag.CLASS_CONTEXT_SPECIFIC) {
				...
				case _ID_pdpTypeOrganization:
					if (!ais.isTagPrimitive())
						throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ".pdpTypeOrganization: Parameter is primitive",
				...
				case _ID_pdpTypeNumber:
					if (!ais.isTagPrimitive())
						throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ".pdpTypeNumber: Parameter is primitive",
								CAPParsingComponentExceptionReason.MistypedParameter);
				...
				case _ID_pdpAddress:
					if (!ais.isTagPrimitive())
						throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ".pdpAddress: Parameter is primitive",
								CAPParsingComponentExceptionReason.MistypedParameter);
		...
		if (this.pdpTypeOrganization == null)
			throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ": pdpTypeOrganization is mandatory but not found",
					CAPParsingComponentExceptionReason.MistypedParameter);
		
		if (this.pdpTypeNumber == null)
			throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName + ": pdpTypeNumber is mandatory but not found",
					CAPParsingComponentExceptionReason.MistypedParameter);
	}
	...
	public void encodeData(AsnOutputStream asnOs) throws CAPException {
		...
		((PDPTypeOrganizationImpl) this.pdpTypeOrganization).encodeAll(asnOs, Tag.CLASS_CONTEXT_SPECIFIC, _ID_pdpTypeOrganization);
