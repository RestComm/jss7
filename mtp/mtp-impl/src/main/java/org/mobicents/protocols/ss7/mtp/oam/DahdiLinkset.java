...
public class DahdiLinkset extends Linkset implements Mtp3Listener {
	...
	public DahdiLinkset(TextBuilder linkSetName, int type) {
	...
	protected void init() {
		...
		if (this.mode == LinksetMode.CONFIGURED) {
			...
			if (this.mtp3 == null) {
				...
				this.mtp3 = new Mtp3(this.linkSetName.toString());
			}

			this.mtp3.clearLinks();
			this.mtp3.addMtp3Listener(this);

			for (FastMap.Entry<TextBuilder, Link> e = this.links.head(), end = this.links
	...
	public boolean addLink(TextBuilder linkName, ByteBuffer byteBuffer) {
		...
		if (links.containsKey(linkName)) {
			byteBuffer.put(ShellMessages.LINK_ALREADY_EXIST);
			return FALSE;
		}
	...
	public boolean noShutdown(ByteBuffer byteBuffer) {
		...
		byteBuffer.put(ShellMessages.LINKSET_NO_LINKS_CONFIGURED);
		return FALSE;
