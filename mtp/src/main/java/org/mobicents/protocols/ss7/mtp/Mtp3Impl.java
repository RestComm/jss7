 ...
 * @author baranowb
 */
public class Mtp3Impl implements Mtp2Listener, Runnable,Mtp3 {
...
//    }
    ...
    public void linkInService(Mtp2 link) {
        ...
        if(link.getSLTMTest() == null)
        {
        	SLTMTestImpl tester = new SLTMTestImpl(link);
        	link.setSLTMTest(tester);
    ...
    private class SLTMTestImpl implements Runnable,SLTMTest {
        private Mtp2 link;
        ...
        private SLTMTestImpl(Mtp2 link) {
            this.link = link;
        }
        ...
        public void ack() {
            ...
            tryCount = 0;
            //shcedule next ping
            testFuture = mtpTimer.schedule(this, Mtp3Impl.TIMEOUT_T2_SLTM, TimeUnit.SECONDS);
            if (logger.isDebugEnabled()) {
                ...
                case LINK_TESTING:
                    ...
                    if (h0 == 1 && h1 == 1) {
                        ...
                        try {
							...
							//something went wrong on L2, link down, no need to do anything?
							//FIXME: add link fail?
							e.printStackTrace();
						}
                    } else if (h0 == 1 && h1 == 2) {
