...
public class Mtp2Impl implements Mtp2{
   ...
   // private long lastWriteStamp = 0;    
    ...
    public Mtp2Impl(String name, int sls) {
        this.name = name;
        this.sls = sls;
...
//        this.listeners.add(layer3);
	...
	public void start() throws IOException {
        ...
        queueLSSU(Mtp2Impl.FRAME_STATUS_INDICATION_OS);
        processTx(16);
	startInitialAlignment();
    }

    public void stop() {
    	listener.unregisterLink(this);
        started = false;
        reset();
        this.channel.close();
        ...
        switch (this.state) {
            ...
            case MTP2_ALIGNED:
                ...
                switch (type) {
                    ...
                    case FRAME_STATUS_INDICATION_E:
                        // 1. set T4 time.
                        this.T4_TIMEOUT = Mtp2Impl.T4_TIMEOUT_EMERGENCY;
                    case FRAME_STATUS_INDICATION_N:
                        // 1. stop T3
                        stop_T3();
                        // 2. determine threashold for AERM.
                        if (this.T4_TIMEOUT == Mtp2Impl.T4_TIMEOUT_EMERGENCY) {
                            this.aermThreshold = Mtp2Impl.AERM_THRESHOLD_EMERGENCY;
                        }
    ...
    private void processMSU(int len) {
        if (this.listener != null) {
            ...
            listener.onMessage(sio, sif, this);
        }
    }
