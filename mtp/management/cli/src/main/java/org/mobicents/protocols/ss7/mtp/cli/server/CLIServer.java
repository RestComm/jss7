package org.mobicents.protocols.ss7.mtp.cli.server;
...
import org.mobicents.protocols.ss7.mtp.cli.AbstractCommand;
import org.mobicents.protocols.ss7.mtp.cli.CLICmdListener;
import org.mobicents.protocols.ss7.mtp.cli.CliCmdEnum;
import org.mobicents.protocols.ss7.mtp.cli.ExitCommand;
import org.mobicents.protocols.ss7.mtp.cli.NoshutdownCommand;
import org.mobicents.protocols.ss7.mtp.cli.SS7Command;
import org.mobicents.protocols.ss7.mtp.cli.ShowCommand;
import org.mobicents.protocols.ss7.mtp.cli.ShutdownCommand;
...
public class CLIServer {
	...
	private static final Logger logger = Logger.getLogger(CLIServer.class);
	...
	private CLICmdListener cLICmdListener = null;
...
//	private long maxBeginsAfter = 1000000;
	...
	public void setCliCmdListener(CLICmdListener cLICmdListener) {
...
//				tick++;
	...
	private void readDataFromSocket(SelectionKey key) throws Exception {
		...
		while (socketChannel.isOpen()
			...
			if (cmd == CliCmdEnum.SHOW.getCmdInt()) {
				shwCmd.decode(body);
				this.write(socketChannel);

			} else if (cmd == CliCmdEnum.SS7.getCmdInt()) {
				ss7Cmd.decode(body);
				this.write(socketChannel);

			} else if (cmd == CliCmdEnum.SHUTDOWN.getCmdInt()) {
				shutDwnCmd.decode(body);
				this.write(socketChannel);

			} else if (cmd == CliCmdEnum.NOSHUTDOWN.getCmdInt()) {
				noShutDwnCmd.decode(body);
				this.write(socketChannel);

			} else if (cmd == CliCmdEnum.EXIT.getCmdInt()) {
				this.stop();
				exitCmd.decode(body);
				this.write(socketChannel);
				socketChannel.close();
