...
public class GT0100SccpStackImplTest extends SccpHarness {
	...
	public void testRemoteRoutingBasedOnGT_DPC_SSN() throws Exception {
		...
		SccpAddress rule1SccpAddress = new SccpAddress(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, 0, new GT0100(0,
				NumberingPlan.ISDN_TELEPHONY, NatureOfAddress.NATIONAL, GT2_pattern_digits), getSSN());
		SccpAddress rule2SccpAddress = new SccpAddress(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, 0, new GT0100(0,
				NumberingPlan.ISDN_MOBILE, NatureOfAddress.NATIONAL, GT1_pattern_digits), getSSN());
		...
		User u1 = new User(sccpStack1.getSccpProvider(), a1, a2, getSSN()) {
			...
			protected boolean matchCalledPartyAddress() {
				...
				SccpAddress addressToMatch = new SccpAddress(RoutingIndicator.ROUTING_BASED_ON_DPC_AND_SSN,
						getStack1PC(), null, getSSN());
		...
		User u2 = new User(sccpStack2.getSccpProvider(), a2, a1, getSSN()) {
			...
			protected boolean matchCalledPartyAddress() {
				SccpMessage msg = messages.get(0);
				SccpDataMessage udt = (SccpDataMessage) msg;
				SccpAddress addressToMatch = new SccpAddress(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE,
						getStack2PC(), new GT0100(0, NumberingPlan.ISDN_TELEPHONY, NatureOfAddress.NATIONAL, "021"),
						getSSN());
				if (!addressToMatch.equals(udt.getCalledPartyAddress())) {
					return false;
				}
	...
	public void testRemoteRoutingBasedOnGT() throws Exception {
		...
		// add addresses to translate
		SccpAddress primary1SccpAddress = new SccpAddress(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE,
		...
		rule1SccpAddress = new SccpAddress(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, 0, new GT0100(0,
				NumberingPlan.ISDN_MOBILE, NatureOfAddress.NATIONAL, "23456/?/8"), getSSN());
		rule2SccpAddress = new SccpAddress(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, 0, new GT0100(0,
				NumberingPlan.ISDN_TELEPHONY, NatureOfAddress.NATIONAL, "02/?"), getSSN());
		...
		User u1 = new User(sccpStack1.getSccpProvider(), a1, a2, getSSN()) {
			...
			protected boolean matchCalledPartyAddress() {
				...
				// 2345678
				SccpAddress addressToMatch = new SccpAddress(RoutingIndicator.ROUTING_BASED_ON_DPC_AND_SSN,
						getStack1PC(), new GT0100(0, NumberingPlan.ISDN_MOBILE, NatureOfAddress.NATIONAL, "2345678"),
						getSSN());
				if (!addressToMatch.equals(udt.getCalledPartyAddress())) {
					return false;
				}
